...
export interface IMenuSubButtonProps extends NestedMenuButtonProps {
  title?: string;
  icon?: LucideIcon;
  arrow?: boolean;
}

export function MenuSubButton({ ...props }: IMenuSubButtonProps) {
  const { title, icon: Icon, arrow, ...rest } = props;

  const rootClass = `
		rounded-md flex grow items-center gap-2 py-1.5 px-2.5 
  ...
  const titleClass = `
		grow text-[0.825rem] font-medium text-start 
