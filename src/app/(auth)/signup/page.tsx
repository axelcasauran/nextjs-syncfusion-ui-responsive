...
import { RecaptchaPopover } from '@reui/recaptcha-popover';
...
export default function SignupPage() {
  ...
  const handleVerifiedSubmit = async (token: string) => {
    try {
      ...
      const response = await fetch('/api/user-management/auth/signup', {
        method: 'POST',
        headers: {
  ...
  if (success) {
    return (
      <Alert appearance="soft" onClose={() => setSuccess(false)}>
        <AlertIcon>
          <Check />
        </AlertIcon>
        <AlertTitle>
      ...
      <Form {...form}>
        <form onSubmit={handleSubmit} className="block w-full space-y-4">
          ...
          <div className="flex flex-col gap-3.5">
            ...
            >
              <Icons.google className="size-4!" /> Sign up with Google
            </Button>
          </div>
                  ...
                  <FormControl>
                    <div className="flex items-center gap-2.5">
                      ...
                      <Link
                        href="/privacy-policy"
                        target="_blank"
                        className="-ms-0.5 text-sm font-semibold text-foreground hover:underline hover:underline-offset-2"
                      >
                        Privacy Policy
                      </Link>
                    </div>
