...
} from '@user-management-app/users/[id]/forms/user-profile-schema';
...
export async function PUT(
  ...
  try {
    ...
    const { firstName, status, roleId }: UserProfileSchemaType = parsedData.data;
    ...
    await prisma.$transaction(async (tx) => {
      const user = await tx.user.update({
        where: { id },
        data: { firstName, status: status as UserStatus, roleId },
      });
